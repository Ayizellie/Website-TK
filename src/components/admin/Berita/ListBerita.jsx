...
import { FaTrash, FaEdit } from "react-icons/fa"; // tambahkan FaEdit
...
const ListBerita = ({ dataBerita, onTambah, onEdit, onHapus }) => {
  ...
  const handleDetail = (item) => {
    Swal.fire({
      title: `<strong>${item.tagline}</strong>`,
      html: `
        <img src="${item.foto}" alt="${item.tagline}" class="w-full h-48 object-cover rounded-lg mb-4"/>
        <p class="text-gray-700 text-sm">${item.isi}</p>
      `,
  ...
  const handleEdit = (item) => {
    Swal.fire({
      title: "Edit Berita",
      text: `Kamu akan mengedit berita: "${item.tagline}"`,
      icon: "info",
  ...
  return (
    <NavbarSidebar>
      <div className="pt-0 px-8 pb-8">
        <h2 className="text-left text-[#064a8c] text-xl font-medium mb-3">Kelola Berita</h2>
        <h2 className="text-left text-[#047DD2] text-sm font-light mb-8">Management Dashboard Admin</h2>
        ...
        <div className="flex flex-col gap-4">
          {dataBerita.map((item) => (
            <div key={item.id} className="flex flex-col sm:flex-row border rounded-xl p-4 bg-white shadow hover:shadow-md transition">
                <img
                    src={item.foto}
                    alt={item.tagline}
                ...
                <div className="flex-1">
                    <h3 className="font-semibold text-gray-800">{item.tagline}</h3>
                    <p className="text-gray-600 text-sm truncate">{item.isi}</p>
